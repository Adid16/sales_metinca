...
<div class="card shadow-sm">
    ...
    <div class="card-body px-4 py-4">
        ...
        {{-- ====================================================================
             NEGOTIATION COUNTER & MANAGER OVERRIDE BADGE (SELALU MUNCUL)
             ==================================================================== --}}
        @php
        ...
        @if(in_array($quotation->status, ['accepted', 'po', 'rejected']))
            @php
            ...
            <div class="mb-3 d-flex align-items-center justify-content-between flex-wrap gap-3 shadow-sm" style="background: linear-gradient(135deg, #d4edda, #c3e6cb); border: 1.5px solid #28a745; border-radius: 10px; padding: 16px;">
                ...
                @if(auth()->check() && auth()->user()->isCustomer())
                    <div>
                        <a href="{{ route('purchase-orders.create', ['quotation_id' => $quotation->id]) }}" class="btn btn-success btn-sm fw-semibold shadow-sm">
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
            ...
            <div class="d-flex gap-2 align-items-center flex-wrap">
                ...
                <a href="{{ route('quotations.export-pdf', $quotation->id) }}" class="btn btn-sm btn-danger" title="Export PDF">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered mb-0 align-middle">
                        ...
                        <tbody>
                            ...
                            @forelse ($quotation->items as $index => $item)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($item->article)
                                            <small class="text-muted">
                                                Art. No: <span class="fw-semibold">{{ $item->article->article_no }}</span>
                                                @if($item->article->material) &bull; Mat: {{ $item->article->material }} @endif
                                            </small>
                                        @endif
                                    </td>
                                    ...
                                    @if($isInternalSales)
                                        <td class="text-end">
                                            @if($floorPrice > 0)
                                                <span class="text-muted fw-semibold">Rp {{ number_format($floorPrice, 0, ',', '.') }}</span>
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif
                                        </td>
                                    ...
                                    <td class="text-end">
                                        ...
                                        @endif
                                        <span class="fw-bold {{ $isBelow ? 'text-danger' : 'text-dark' }}">
                                            Rp {{ number_format($displayPrice, 0, ',', '.') }}
                                        </span>
                                        @if($isInternalSales && $isBelow)
                            ...
                            @empty
                                <tr><td colspan="{{ $isInternalSales ? 7 : 6 }}" class="text-center text-muted fst-italic py-3">Tidak ada item</td></tr>
                            @endforelse
                        </tbody>
                        <tfoot style="background: #f0f4ff;">
                            <tr>
                                <th colspan="{{ $isInternalSales ? 6 : 5 }}" class="text-end py-2">{{ $lastNego ? 'NEGOTIATED GRAND TOTAL' : 'GRAND TOTAL' }}</th>
        ...
        @if($lastNegotiation)
            <div class="card border mb-4">
                <div class="card-header px-3 py-2 d-flex justify-content-between align-items-center" style="background: #fff3cd;">
                    ...
                    <small class="text-muted">
                        {{ $lastNegotiation->from_customer ? ($quotation->customer->name ?? 'Customer') : 'PT. Metinca Prima' }} &middot; {{ $lastNegotiation->created_at->format('d M Y, H:i') }}
                    </small>
                </div>
