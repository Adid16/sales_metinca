            ...
            <table class="table table-hover" id="table1">
                ...
                <tbody>
                    @forelse ($quotations as $quotation)
                        ...
                        <td><center><span class="badge badge-sm bg-primary"> {{ $quotation->id }}</span></center></td>
                        <td><center>{{ $quotation->quotation_no }}</center></td>
                        <td><center>{{ $quotation->customer->company }}</center></td>
                        {{-- <td><center>{{ $quotation->attachments }}</center></td> --}}
                        <td><center>{{ $quotation->date_expired }}</center></td>
                        <td><center>
                        ...
                        <td><center>
                            ...
                            {{-- @if(auth()->user()->isAdmin() && $quotation->status == 'created')
                            ...
                            {{-- BUTTON INI HARUS BISA NGIRIM NIH KING KE LOGINNYA CUSTOMER --}}
                            ...
                            <form method="POST" action="{{route('quotations.send',$quotation->id)}}" class="d-inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit" onclick="return confirm('Apakah anda yakin ingin mengirim quotation ini?')" class="btn btn-sm btn-primary"><i class="bi bi-send-fill"></i></button>
                            </form>
                            @endif
                        </center></td>
                        <th><center>
                            {{ $quotation->status }}
                            {{-- KING, DISINI MAININ STATUS YAA, STATUS YANG REAL TIME SETIAP AKSI PLEASE
                                JANGAN GA ADA STATUS, GAENAK HTS TAU GUA PERNAH. UNTUNG SEKARANG DAH PUNYA COWO
                                #PACARHOLIC #COWOKUHOLIC
                                ENIH STATUSNYA GINI AJA : 1. CREATED (DATE)
                                                          2. SENT (DATE)
                                                          3. ACCEPTED (DATE)
                                                          4. PO (DATE)
                                                          --}}
