<?php $__env->startSection('title', 'PT. Metinca Prima Industrial Works'); ?>


<?php $__env->startPush('styles'); ?>
    <link rel="shortcut icon" href="./assets/compiled/svg/favicon.svg" type="image/x-icon">
    <link rel="shortcut icon" href="data:image/png;base64,%." type="image/png">
    <link rel="stylesheet" href="assets/extensions/simple-datatables/style.css">
    <link rel="stylesheet" href="./assets/compiled/css/table-datatable.css">
    <link rel="stylesheet" href="./assets/compiled/css/app.css">
    <link rel="stylesheet" href="./assets/compiled/css/app-dark.css">
    <link rel="stylesheet" href="<?php echo e(asset('assets/css/statustabel.css')); ?>">
<?php $__env->stopPush(); ?>


<?php $__env->startSection('content'); ?>
<div class="card detail-card">
    <div class="card-header bg-primary text-white py-3">
        <h5 class="card-title mb-0">
            <i class="bi bi-file-earmark-text-fill"></i> Quotations
        </h5>
    </div>

<section class="section">
    <?php if(session('success')): ?>
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            <i class="bi bi-check-circle"></i> <?php echo e(session('success')); ?>

            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php endif; ?>

    <?php if(session('error')): ?>
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <i class="bi bi-exclamation-triangle"></i> <?php echo e(session('error')); ?>

            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    <?php endif; ?>

    <div class="card-body py-0">
        <form class="mb-3" method="GET" action="<?php echo e(route('quotations.index')); ?>">
            <div class="row g-2 align-items-end mt-0">
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="d-flex align-items-center gap-1">
                        <label class="form-label small mb-0 text-nowrap">From : </label>
                        <input type="date" name="start_date" class="form-control form-control-sm" value="<?php echo e($filters['start_date'] ?? ''); ?>" placeholder="From">
                    </div>
                </div>
        
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="d-flex align-items-center gap-1">
                        <label class="form-label small mb-0 text-nowrap">To : </label>
                        <input type="date" name="end_date" class="form-control form-control-sm" value="<?php echo e($filters['end_date'] ?? ''); ?>" placeholder="To">                      
                    </div>
                </div>

                <div class="col-12 col-sm-6 col-lg-2">
                    <div class="d-flex align-items-center gap-1">
                        <select name="status" class="form-select form-select-sm">
                            <option value="">All Status</option>
                            <option value="created" <?php echo e((isset($filters['status']) && $filters['status']=='created') ? 'selected' : ''); ?>>Created</option>
                            <option value="sent" <?php echo e((isset($filters['status']) && $filters['status']=='sent') ? 'selected' : ''); ?>>Sent</option>
                            <option value="negotiating" <?php echo e((isset($filters['status']) && $filters['status']=='negotiating') ? 'selected' : ''); ?>>Negotiating</option>
                            <option value="accepted" <?php echo e((isset($filters['status']) && $filters['status']=='accepted') ? 'selected' : ''); ?>>Accepted</option>
                            <option value="po" <?php echo e((isset($filters['status']) && $filters['status']=='po') ? 'selected' : ''); ?>>PO</option>
                        </select>
                    </div>
                </div>

                <div class="col-12 col-sm-6 col-lg-4 d-flex flex-wrap gap-1">
                    <button type="submit" class="btn btn-sm btn-secondary">Filter</button>
                    <button type="submit" formaction="<?php echo e(route('quotations.export')); ?>" class="btn btn-success btn-sm">Export</button>
                    <?php if(!auth()->user()->isCustomer()): ?>
                        <a href="<?php echo e(route('quotations.create')); ?>" class="btn btn-primary btn-sm"> Add Quotation</a>
                    <?php endif; ?>  
                </div>
            </div>
        </form>

        <div class="table-responsive">
        <table class="table table-hover align-middle" id="table1">
            <thead class="table-light">
                <tr>
                    <th class="text-center" width="5%">No</th>
                    <th class="text-center">Req ID</th>
                    <th>No Quotation</th>
                    <th>Customer / Perusahaan</th>
                    <th class="text-center">Tgl Quotation</th>
                    <th class="text-center">Tgl Expired</th>
                    <th class="text-center">Sales PIC</th>
                    <th class="text-center" width="18%">Aksi</th>
                    <th class="text-center" width="12%">Status</th>
                </tr>
            </thead>
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $quotations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $quotation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr>
                        <td class="text-center fw-bold"><?php echo e($loop->iteration); ?></td>
                        <td class="text-center">
                            <span class="badge bg-light-primary text-primary fw-bold"><?php echo e($quotation->request_id ?? '-'); ?></span>
                        </td>
                        <td class="fw-bold text-primary"><?php echo e($quotation->quotation_no); ?></td>
                        <td>
                            <strong><?php echo e($quotation->customer->company ?? $quotation->customer->name ?? '-'); ?></strong>
                            <?php if(!empty($quotation->customer->name) && !empty($quotation->customer->company)): ?>
                                <br><small class="text-muted"><i class="bi bi-person me-1"></i><?php echo e($quotation->customer->name); ?></small>
                            <?php endif; ?>
                        </td>
                        <td class="text-center small"><?php echo e(\Carbon\Carbon::parse($quotation->created_at)->format('d M Y')); ?></td>
                        <td class="text-center small">
                            <?php echo e($quotation->date_expired ? \Carbon\Carbon::parse($quotation->date_expired)->format('d M Y') : '-'); ?>

                        </td>
                        <td class="text-center">
                            <?php if($quotation->request && $quotation->request->assignment): ?>
                                <span class="badge bg-success">
                                    <?php echo e($quotation->request->assignment->sales->name ?? 'N/A'); ?>

                                </span>
                            <?php else: ?>
                                <span class="text-muted small">-</span>
                            <?php endif; ?>    
                        </td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center align-items-center gap-1">
                                
                                <a href="<?php echo e(route('quotations.show', $quotation->id)); ?>" 
                                   class="btn btn-sm btn-info text-white fw-bold px-3 py-1" 
                                   title="Buka Detail Lengkap Quotation & Data PO Terhubung">
                                    <i class="bi bi-eye-fill me-1"></i> Detail
                                </a>

                                
                                <?php if($quotation->canSend() && auth()->user()->isStaff() && auth()->user()->divisi == 'sales'): ?>
                                    <form method="POST" action="<?php echo e(route('quotations.send', $quotation->id)); ?>" class="d-inline">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('PATCH'); ?>
                                        <button type="submit" onclick="return confirm('Apakah anda yakin ingin mengirim quotation ini ke Customer?')" class="btn btn-sm btn-primary px-2 py-1" title="Kirim Ke Customer">
                                            <i class="bi bi-send-fill"></i> Kirim
                                        </button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        </td>
                        <td class="text-center">
                            <?php $lastNego = $quotation->negotiates?->first(); ?>

                            <?php if($quotation->status == 'created'): ?>
                                <span class="badge bg-secondary">Created</span>
                                <br><small class="text-muted" style="font-size:10px">
                                    <?php echo e(\Carbon\Carbon::parse($quotation->created_at)->format('d M Y')); ?>

                                </small>

                            <?php elseif($quotation->status == 'sent'): ?>
                                <span class="badge bg-info text-dark">Sent</span>
                                <br><small class="text-muted" style="font-size:10px">
                                    <?php echo e(\Carbon\Carbon::parse($quotation->updated_at)->format('d M Y')); ?>

                                </small>

                            <?php elseif($quotation->status == 'negotiating'): ?>
                                <span class="badge bg-warning text-dark">Negotiating</span>
                                <?php if($lastNego): ?>
                                    <br><small class="text-muted" style="font-size:10px">
                                        <?php echo e($lastNego->created_at->format('d M Y')); ?>

                                    </small>
                                <?php endif; ?>

                            <?php elseif($quotation->status == 'accepted'): ?>
                                <span class="badge bg-success">
                                    <i class="bi bi-check-circle-fill me-1"></i>Accepted
                                </span>
                                <br><small class="text-muted" style="font-size:10px">
                                    <?php echo e($quotation->accepted_date ? \Carbon\Carbon::parse($quotation->accepted_date)->format('d M Y') : '-'); ?>

                                </small>

                            <?php elseif($quotation->status == 'po'): ?>
                                <span class="badge bg-primary">PO</span>

                            <?php else: ?>
                                <span class="badge bg-secondary"><?php echo e($quotation->status); ?></span>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="9" class="text-center text-muted py-4">Belum ada data quotation.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
        </div>
    </div>
</section>

<?php $__env->startPush('scripts'); ?>
    <script src="<?php echo e(asset('assets/extensions/simple-datatables/umd/simple-datatables.js')); ?>"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            if (document.querySelector("#table1")) {
                let dataTable = new simpleDatatables.DataTable("#table1");
            }
        });
    </script>
<?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sales_metinca\resources\views/quotations/index.blade.php ENDPATH**/ ?>