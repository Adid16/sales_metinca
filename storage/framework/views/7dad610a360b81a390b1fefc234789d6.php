<?php $__env->startSection('title', 'Edit Contract Review Sheet - PT. Metinca Prima Industrial Works'); ?>
    ...
    <div class="card border-danger border-2 shadow-sm mb-4 alert-permanent">
        ...
        <div class="card-body bg-danger-subtle p-3">
            <ul class="mb-2 list-unstyled">
                <?php $__currentLoopData = $rejections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mgr => $rsn): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li class="p-2 mb-2 bg-white rounded border border-danger-subtle text-dark">
                        <strong class="text-danger"><i class="bi bi-person-x-fill me-1"></i><?php echo e($mgr); ?>:</strong>
                        <span class="fst-italic fw-semibold ms-1 text-dark">"<?php echo e($rsn); ?>"</span>
                    </li>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </ul>
...
<form action="<?php echo e(route('contracts.update', $contract->id)); ?>" method="POST" enctype="multipart/form-data">
    <?php echo csrf_field(); ?>
        ...
        <div class="row match-height">
            <div class="col-12">
                ...
                <div class="card-body px-4 py-3">
                    ...
                    <input type="hidden" name="customer_id" value="<?php echo e($contract->customer_id); ?>">
                    <input type="hidden" name="quotation_id" value="<?php echo e($contract->quotation_id); ?>">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control form-control-sm text-uppercase fw-semibold" id="partNumber" name="part_no" value="<?php echo e($contract->part_no ?? ($contract->article->internal_part_no ?? ($contract->article->part_number ?? ($contract->article->article_no ?? ($contract->internalItem->part_no ?? ''))))); ?>" readonly>
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="hidden" name="amandement_no" value="<?php echo e($suggestedAmandement); ?>">
                            <input type="text" class="form-control form-control-sm" value="<?php echo e($displayText); ?>" readonly>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label mb-1 fw-semibold small">PART NAME</label>
                            <input type="text" class="form-control form-control-sm" id="partName" name="part_name" value="<?php echo e(old('part_name', $contract->part_name)); ?>" readonly>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label mb-1 fw-semibold small">LOCATION</label>
                            <input type="text" class="form-control form-control-sm" id="location" name="location" value="<?php echo e(old('location', $contract->article->lokasi_text ?? '-')); ?>" readonly>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label mb-1 fw-semibold small">ARTICLE</label>
                            <input type="hidden" id="articleId" name="article_id" value="<?php echo e($contract->article_id); ?>">
                            <input type="text" class="form-control form-control-sm" id="articleInput" placeholder="Ketik kode article" value="<?php echo e(old('article_no', $contract->article->article_no ?? '-')); ?>">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card shadow-sm mb-3">
                    <div class="card-header d-flex justify-content-between align-items-center py-2"
                        ...
                        <h6 class="mb-0 fw-bold text-uppercase text-white"
                            ...
                            <i class="bi <?php echo e($icon); ?> me-1"></i><?php echo e($deptLabels[$dept] ?? strtoupper($dept)); ?>

                        </h6>

                        <div>
                            <?php if($isDeptApproved): ?>
                                <span class="badge bg-success" style="font-size: 0.75rem;">
                                    <i class="bi bi-check-circle-fill me-1"></i>Sudah Disetujui (Terkunci)
                                </span>
                            <?php elseif(!empty($deptRejectReason)): ?>
                    ...
                    <?php if(!empty($deptRejectReason)): ?>
                        <div class="alert alert-danger alert-permanent m-2 py-2 px-3 small border border-danger-subtle bg-danger-subtle text-danger-emphasis rounded">
                            <i class="bi bi-chat-left-quote-fill me-1 text-danger"></i> <b>Catatan Penolakan Manager:</b> <span class="fst-italic fw-semibold text-dark">"<?php echo e($deptRejectReason); ?>"</span>
                        </div>
                    <?php endif; ?>
                        ...
                        <table class="table table-bordered align-middle mb-0">
                            ...
                            <tbody class="requirement-body" data-dept="<?php echo e($dept); ?>">
                                ...
                                <tr>
                                    <td>
                                        <input type="hidden"
                                            name="requirements[<?php echo e($req->id); ?>][id]"
                                            value="<?php echo e($req->id); ?>">

                                        <input type="text"
                                            name="requirements[<?php echo e($req->id); ?>][requirement]"
                                            class="form-control form-control-sm <?php echo e($isDefault || $isDeptApproved ? 'bg-light fw-semibold text-dark' : ''); ?>"
                                            value="<?php echo e($req->requirement); ?>"
                                            <?php echo e($isDefault || $isDeptApproved ? 'readonly' : ''); ?>>
                                    </td>
                                    <td>
                                        <input type="text"
                                            name="requirements[<?php echo e($req->id); ?>][value]"
                                            class="form-control form-control-sm <?php echo e($isDeptApproved ? 'bg-light text-muted' : ''); ?>"
                                            value="<?php echo e($req->requirement_value); ?>"
                                            placeholder="Keterangan / Value"
                                            <?php echo e($isDeptApproved ? 'readonly' : ''); ?>>
                                    </td>
                                    <td class="text-center">
                                        <?php if(!$isDefault && !$isDeptApproved): ?>
            ...
            <div class="card shadow-sm mb-3">
                ...
                <div class="card-body">
                    <textarea name="others_comment" class="form-control mt-2" rows="3"
                        placeholder="Tulis komentar tambahan..."><?php echo e($contract->others_comment); ?></textarea>
                        ...
                        <?php if($contract->po_pdf): ?>
                            <div class="mb-2 p-2 border rounded bg-light d-flex justify-content-between align-items-center" style="font-size: 12px;">
                                <span>
                                    ...
                                    <a href="<?php echo e(asset('storage/' . $contract->po_pdf)); ?>" target="_blank" class="fw-bold text-primary text-decoration-underline">
                                        Lihat PDF Terupload
                                    </a>
                                </span>
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-4">
                        <a href="<?php echo e(route('contracts.show', $contract->id)); ?>" class="btn btn-sm btn-light border">Batal</a>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sales_metinca\resources\views/contracts/edit.blade.php ENDPATH**/ ?>